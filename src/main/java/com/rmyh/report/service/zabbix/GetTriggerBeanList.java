...
//	public static int itemId;
	...
	public static List<TriggerBean> getBean() throws ZabbixApiException {
		...
		// TriggerPrototypeGetResponse triggerObj = new GetTriggerResponse().getProtoTrigger();
	...
	public static String transExpression(List <ItemBean> allItemBean,String string) throws NumberFormatException, ZabbixApiException {
...
//		List <ItemBean> allItemBean= new GetItems().getAllItems(); 
		for(int i=0;m.find();i++) {
			if(true) {
				String Replace = transItemId2Name(allItemBean,Integer.parseInt(m.group().substring(m.group().indexOf("{")+1,m.group().indexOf("}"))));
